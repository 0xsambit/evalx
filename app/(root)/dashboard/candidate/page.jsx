	...
	return (
		<>
			<section className='w-full h-full bg-[#F8FAFC] px-10 font-poppins'>
				...
				<div className='flex items-center justify-around m-auto flex-wrap'>
					{insights.slice(3, 4).map((card) => (
						<div
							...
							<h1 className='text-base font-medium mb-2'>{card.title}</h1>
							<p className='text-3xl font-bold'>{card.value}</p>
						</div>
					))}
					<div className='flex justify-center items-center gap-5'>
						<Image src={pfp} alt='profile' className='size-32 rounded-full' />
						<div>
							<h1 className='text-lg font-medium'>{session.user.name}</h1>
							<h1 className='text-lg font-regular'>{session.user.email}</h1>

							<div className='flex justify-center items-center gap-5'>
								<Link href={`/user/${session.user.name}`} passHref>
				...
				<h1 className='text-2xl font-semibold my-5'>Your recent interviews -</h1>
						...
						<div className='flex justify-between items-center gap-10 px-5'>
							<div className='flex justify-center items-center gap-5'>
								...
								<div className='py-5 flex flex-col justify-center items-start gap-2'>
									<h1 className='font-medium text-white'>{e.title}</h1>
									<p className='text-[#ababab]'>{e.date}</p>
									<p className='text-[#ababab]'>Job Role: Full Stack Developer</p>
								</div>
							</div>
							<h1 className='text-white font-semibold text-lg'>Result: Selected</h1>
							<Dialog>
								<DialogTrigger asChild>
									<button className='button text-black'>View Feedback</button>
								</DialogTrigger>
								<DialogContent className='sm:max-w-md bg-black text-white'>
									<DialogHeader>
										...
										<div className='text-white font-extralight leading-7'>
											<div>
												Great job on the interview! You demonstrated strong problem-solving
												skills and a good understanding of design principles. However, there
												are a few areas for improvement:
											</div>
											<ul className='list-disc list-inside'>
												<li>Consider improving the user flow to make it more intuitive.</li>
												<li>Pay attention to the alignment and spacing of elements.</li>
												<li>Try to use more consistent color schemes.</li>
											</ul>
											<p>
												Overall, we were impressed with your performance and look forward to
				...
				<h1 className='text-2xl font-semibold my-5'>Performance Insights</h1>
				<Chart />
